...
-----------------------------------------
local function delmsg (Venus,ZakariaR)
    msgs = Venus.msgs 
    for k,v in pairs(ZakariaR.messages_) do
        msgs = msgs - 1
        tdcli.deleteMessages(v.chat_id_,{[0] = v.id_}, dl_cb, cmd)
        if msgs == 1 then
            tdcli.deleteMessages(ZakariaR.messages_[0].chat_id_,{[0] = ZakariaR.messages_[0].id_}, dl_cb, cmd)
            return false
        end
    end
    tdcli.getChatHistory(ZakariaR.messages_[0].chat_id_, ZakariaR.messages_[0].id_,0 , 100, delmsg, {msgs=msgs})
end
-----------------------------------------
...
------------------------------------------
    if (matches[1]:lower() == "rmsg" or matches[1] == "پاکسازی") and is_mod(msg) then
        if tostring(msg.to.id):match("^-100") then 
            ...
            return '⚠️ _این قابلیت فقط در سوپرگروه ممکن است_ '
